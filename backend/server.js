...
app.get("/expenditure/items/:userId", async (req, res) => {
  ...
  connection.query(
    "SELECT * FROM expenditure where user_id = ?",
    [userId],
...
app.get("/expenditure/items/month/:diff/:userId", async (req, res) => {
  ...
  const queryString =
    "SELECT * FROM expenditure WHERE TIMESTAMPDIFF(MONTH, monthAndYear, CURDATE()) > 0 AND TIMESTAMPDIFF(MONTH, monthAndYear, CURDATE()) <= ? " +
...
app.get("/expenditure/items/amount/:diff/:userId", async (req, res) => {
  ...
  console.log("Diff: " + diff);
  const queryString =
    "SELECT *, SUM(amount) as amount FROM expenditure WHERE TIMESTAMPDIFF(MONTH, monthAndYear, CURDATE()) > 0 AND TIMESTAMPDIFF(MONTH, monthAndYear, CURDATE()) <= ? " +
...
app.get("/expenditure/items/category/total/:userId", (req, res) => {
  const { userId } = req.params;
  connection.query(
    "SELECT expenseType, SUM(amount) AS total_amount FROM expenditure WHERE YEAR(monthAndYear) = YEAR(CURDATE()) AND MONTH(monthAndYear) = MONTH(CURDATE()) " +
      "AND user_id = ? GROUP BY expenseType, MONTH(monthAndYear) ORDER by month(monthAndYear) DESC",
    [userId],
    (error, results) => {
      if (error) {
        res.status(500).json({ error: "Internal server error" });
...
app.get("/expenditure/items/:item/:userId", (req, res) => {
  ...
  connection.query(
    "SELECT * FROM expenditure WHERE expenseType = ? AND YEAR(monthAndYear) = YEAR(CURDATE()) AND MONTH(monthAndYear) = MONTH(CURDATE()) AND user_id = ?",
    [item, userId],
    (error, results) => {
      if (error) {
...
app.get("/expenditure/items/expenditure/total/:userId", (req, res) => {
  ...
  connection.query(
    "SELECT SUM(amount) as total_expenditure FROM expenditure WHERE YEAR(monthAndYear) = YEAR(CURDATE()) AND MONTH(monthAndYear) = MONTH(CURDATE()) AND user_id = ?",
    [userId],
    (error, results) => {
      if (error) {
        console.error("Error retrieving: " + error);
...
app.post("/expenditure/items/:userId", (req, res) => {
  ...
  connection.query(
    "INSERT INTO expenditure (monthAndYear, expenseType, amount, user_id) VALUES (?, ?, ?, ?)",
    [date, expenseType, amount, userId],
    (error, results) => {
      if (error) {
...
app.put("/expenditure/items/:item/:userId", async (req, res) => {
  ...
  connection.query(
    "UPDATE expenditure SET amount = ? WHERE expenseType = ? AND user_id = ? ",
    [amount, item, userId],
    async (error, results) => {
      if (error) {
...
app.delete("/expenditure/items/:item/:userId", (req, res) => {
  ...
  connection.query(
    "DELETE FROM expenditure WHERE expenseType = ? AND user_id = ?",
    [item, userId],
    (error, results) => {
      if (error) {
...
app.get("/earnings/month/:month/:userId", (req, res) => {
  ...
  connection.query(
    "SELECT amount FROM earnings WHERE month = ? AND user_id = ?",
    [month, userId],
    (error, results) => {
      if (error) {
        console.error("Error retrieving: " + error);
...
app.get("/earnings/month/:month/year/:year/:userId", (req, res) => {
  ...
  console.log(
    " Server.js Month: " + month + "year: " + year + " user id: " + userId
  );
  connection.query(
    "SELECT amount FROM earnings WHERE month = ? AND year = ? AND user_id = ?",
    [month, year, userId],
    (error, results) => {
      if (error) {
        console.error("Error retrieving: " + error);
      ...
      if (results.length === 0) {
        res
          .status(404)
          .json({ error: "Data not found with this month and year" });
        return;
      }
...
app.get("/earnings/:diff/:userId", async (req, res) => {
  ...
  console.log("User id:  " + userId);
  const queryString =
    "SELECT year, month, amount, user_id, month_difference FROM ( " +
...
app.post("/earnings/:userId", (req, res) => {
  ...
  if (!months || !amount || !years) {
    return res
      .status(400)
      .json({ error: "Month, year and amount are required" });
  }

  // Insert new earnings data into the database
  connection.query(
    "INSERT INTO earnings (month, amount, year, user_id) VALUES (?,?,?,?)",
    [months, amount, years, userId],
    (error, results) => {
      if (error) {
        console.error("Error inserting into database:", error);
        res.status(500).json({ error: "Internal server error" });
...
app.put("/earnings/month/:month/year/:year/:userId", async (req, res) => {
  ...
  connection.query(
    "UPDATE earnings SET amount = ? WHERE month = ? AND year = ? AND user_id = ?",
    [amount, month, year, userId],
    async (error, results) => {
      if (error) {
        console.error("Error updating database: " + error);
        res.status(500).json({ error: "Internal server error" });
...
app.delete("/earnings/:month/:userId", (req, res) => {
  ...
  connection.query(
    "DELETE FROM earnings WHERE month = ? AND user_id = ?",
    [month, userId],
    (error, results) => {
      if (error) {
        console.error("Error deleting from database:", error);
...
app.get("/budget/items/:userId", async (req, res) => {
  const { userId } = req.params;
  connection.query(
    "SELECT * FROM allocated_budget WHERE user_id = ?",
    [userId],
    function (error, results, fields) {
      if (error) {
        console.log("Error: " + error);
...
app.get("/budget/items/month/:diff/:userId", async (req, res) => {
  ...
  const queryString =
    "SELECT * FROM allocated_budget WHERE TIMESTAMPDIFF(MONTH, monthAndYear, CURDATE()) > 0 AND TIMESTAMPDIFF(MONTH, monthAndYear, CURDATE()) <= ? " +
    " AND user_id = ? ORDER BY monthAndYear DESC;";
  connection.query(
    queryString,
    [diff, userId],
...
app.get("/budget/items/amount/:diff/:userId", async (req, res) => {
  const { diff, userId } = req.params;
  const queryString =
    "SELECT *, SUM(amount) as amount FROM allocated_budget WHERE TIMESTAMPDIFF(MONTH, monthAndYear, CURDATE()) > 0 AND TIMESTAMPDIFF(MONTH, monthAndYear, CURDATE()) <= ? " +
    " AND user_id = ? GROUP BY MONTH(monthAndYear) ORDER BY monthAndYear DESC;";
  connection.query(
    queryString,
    [diff, userId],
...
app.get("/budget/items/:item/:userId", (req, res) => {
  ...
  const { item, userId } = req.params;
  connection.query(
    "SELECT value FROM allocated_budget WHERE expenseType = ? AND user_id = ?",
    [item, userId],
    (error, results) => {
      if (error) {
        console.error("Error retrieving: " + error);
...
app.get("/budget/items/budget/total/:userId", (req, res) => {
  ...
  connection.query(
    "SELECT SUM(amount) as total_expense FROM allocated_budget WHERE YEAR(monthAndYear) = YEAR(CURDATE()) AND MONTH(monthAndYear) = MONTH(CURDATE()) AND user_id = ?",
    [userId],
    (error, results) => {
      if (error) {
        console.error("Error retrieving: " + error);
...
app.post("/budget/items/:userId", (req, res) => {
  ...
  if (!expenseType || amount <= 0) {
    return res
      .status(400)
      .json({ error: "Both budget type and amount are required" });
  }

  // Insert new budget data into the database
  connection.query(
    "INSERT INTO allocated_budget (monthAndYear, expenseType, amount, user_id) VALUES (?, ?, ?, ?)",
    [date, expenseType, amount, userId],
    (error, results) => {
      if (error) {
        console.error("Error inserting into database:", error);
...
app.put("/budget/items/:item/:userId", async (req, res) => {
  ...
  console.log(
    " put budget: Label : " + item + " params label: " + req.params.item
  );
  if (!amount) {
    return res.status(400).json({ error: "Value is required" });
  }
  ...
  connection.query(
    "UPDATE allocated_budget SET amount = ? WHERE expenseType = ? AND user_id = ?",
    [amount, item, userId],
    async (error, results) => {
      if (error) {
        console.error("Error updating database: " + error);
...
app.delete("/budget/items/:item/:userId", (req, res) => {
  ...
  connection.query(
    "DELETE FROM allocated_budget WHERE expenseType = ? AND user_id = ?",
    [item, userId],
    (error, results) => {
      if (error) {
        console.error("Error deleting from database:", error);
